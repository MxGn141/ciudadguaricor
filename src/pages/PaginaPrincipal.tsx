...
const secciones = [
  { nombre: 'Nacionales', color: 'bg-guarico-blue', limite: 3 },
  { nombre: 'Municipales', color: 'bg-guarico-green', limite: 3 },
  { nombre: 'DEPORTES', color: 'bg-guarico-gold', limite: 3 },
  { nombre: 'Cultura', color: 'bg-guarico-blue', limite: 3 },
  { nombre: 'Economía', color: 'bg-guarico-green', limite: 3 },
  { nombre: 'Sociales', color: 'bg-guarico-gold', limite: 3 },
  { nombre: 'Sucesos', color: 'bg-guarico-blue', limite: 3 }
];
            ...
            return (
              <section key={seccion.nombre} className="mb-8">
                <div className="flex items-center justify-between mb-4">
                  <h2 className={`text-xl font-bold text-white px-4 py-2 ${seccion.color} inline-block rounded-lg`}>
                    {seccion.nombre}
                  </h2>
                  <Link 
                    to={`/seccion/${seccion.nombre}`}
                    className="bg-guarico-blue text-white px-4 py-1 text-sm rounded-lg hover:bg-guarico-light-blue transition-colors"
                  >
                    VER MÁS
                  </Link>
                </div>
                  ...
                  {noticiasSeccion.map((noticia) => (
                    <div key={noticia.id} className="bg-white rounded-lg shadow-sm overflow-hidden">
                      <Link to={`/noticia/${noticia.id}`} className="block group">
                        ...
                        />
                        <div className="p-4">
                          <h3 className="font-bold text-gray-900 mb-2 group-hover:text-guarico-blue transition-colors line-clamp-2">
                            {noticia.titulo}
                          </h3>
